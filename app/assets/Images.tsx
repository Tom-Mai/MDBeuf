import hyginealimentaire from "./hyginealimentaire.webp";
import lvlreactcorp from "./lvlreactcorp.webp";
import repres1 from './repres1.webp';
import repres2 from './repres2.webp';
import repres3 from './repres3.webp';
import repres4 from './repres4.webp';
import repres5 from './repres5.webp';
import repres6 from './repres6.webp';
import repres7 from './repres7.webp';
import repres8 from './repres8.webp';
import repres9 from './repres9.webp';
import repres10 from './repres10.webp';
import MaurineDubeuf from "./MaurineDubeuf.jpg";
import BannerAboutme from "./BannerAboutme.webp";
import BannerHomePage from "./BannerHomePage.webp";
import BannerPrestations from "./BannerPrestations.webp";
import BannerContact from "./BannerContact.webp";
import photoOfNature from "./photoOfNature.webp";
import handsphoto1 from "./handsphoto1.webp";
import photohandfruits from "./photohandfruits.webp";

export const Images = {
    Maurinedb :[
        {
            src: MaurineDubeuf,
            alt: 'Photo portrait de Maurine Dubeuf, Naturopathe'
        }
    ],
    hyginealimentaire: [
      {
        src:    hyginealimentaire,
        alt: 'Représentation Hygiene alimentaire',
    },
    {
        src:photoOfNature,
        alt:"test",
    },
    {
        src:handsphoto1,
        alt:"hand photo of nature"
    },
    {
        src:photohandfruits,
        alt:"hand photo of nature"
    },
],
Banner:[
    {
        src:BannerAboutme,
        alt:'bathed in warm, natural light, emphasizing tranquility and wellness.',
    },
    {
        src:BannerHomePage,
        alt:' tranquil and inviting, with lighter tones and reduced shadows to create a warm and welcoming atmosphere.'
    },{
        src:BannerPrestations,
        alt:"essence of tranquility, health, and naturopathy."
    },
    {
        src:BannerContact,
        alt: 'Contact naturopathe'
    }
],
lvlreactcorp:[
    {
        src:lvlreactcorp,
        alt:'Représentation des niveaux de reaction corporel'
    }
],
    repres:[
        {
        src:repres1,
        alt:'systeme, nerveux',
    },
    {
        src:repres2,
        alt:'asthme, sinusite, essouflement ',
    },
    {
        src:repres3,
        alt:',cystite lithiase',
    },
    {
        src:repres4,
        alt:'arthrose, osteoporose, polyarthrite, rhumatoïde',
    },
    {
        src:repres5,
        alt:'diarrhée / constipation, ballonnement, remontée acide, maladie de Crohn',
    },
    {
        src:repres6,
        alt:`baisse d'énergie, baisse de l'immunité`,
    },
    {
        src:repres7,
        alt:'eczéma,psoriasis,acné',
    },
    {
        src:repres8,
        alt:'hypothyroïdie, syndrome de Cushing, diabète',
    },
    {
        src:repres9,
        alt:'varice',
    },
    {
        src:repres10,
        alt:'endométriose, SOPK, syndrome pré-menstuel, adénome de la prostate',
    },
    ]};
   

